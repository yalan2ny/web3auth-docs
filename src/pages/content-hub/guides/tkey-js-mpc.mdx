---
title: Using Web3Auth tKey MPC SDK
image: "content-hub/guides/banners/tkey-mpc.png"
...
<SEO
  ...
  image="https://web3auth.io/docs/content-hub/guides/banners/tkey-mpc.png"
  slug="/content-hub/guides/tkey-js-mpc"
/>
...
This is an advanced guide, meant for enterprise developers and contains multiple moving parts that might be difficult to understand. Please contact
our team if you're looking to integrate this SDK, and we'll be happy to help you out.

For general integrations, [MPC Core Kit](/sdk/core-kit/mpc-core-kit) is the best alternative to be used.
...
For this guide, we will be talking through the Web3Auth tKey JS MPC SDK and showing you a basic example of using it with `@tkey-mpc/default` package.
...
- Create a **Verifier** from the **Custom Auth** Section of the [Web3Auth Developer Dashboard](https://dashboard.web3auth.io/) with following
  ...
  - Select environment: Please note the verifier will be deployed on `sapphire_mainnet` or `sapphire_devnet` environment.
...
any other Web3Auth [Guides](./) / [Examples](/examples). Just modify the functions to make it work.
...
For this project, we will be using the `@tkey-mpc/default` package alongside multiple helper packages, that contain all the needed functionalities of
...
refer to our [tKey JS SDK Reference](/sdk/core-kit/tkey) for more details on the conventional functions involved.
...
[Github Example](https://raw.githubusercontent.com/tkey/tkey-mpc-examples/main/tkey-mpc-web/tkey-mpc-react-popup-example/src/utils.ts) or the code
below.
...
Once you have triggered the login process, you're ready to initialize the tKey. This will generate you a Metadata Key corresponding to your login
...
The Web3Auth Metadata stores the information about the user's key locations, like how many keys are generated, which factorPub represents which key
...
function called `addFactorKeyMetadata` which is used to add data in the metadata, you can customise that according to your needs.

By default, if the key is not present, the metadata returns a `KEY_NOT_FOUND` message. We can use this to check if the user is new or existing.
...
Since we are use the tKey Core MPC SDK, we need to implement the device storage functions ourselves. We'll be using the local storage to store the
...
The following code snippet shows how to copy the local TSS share into a manual backup factor key. This is useful if you want to create a backup of
...
The following code snippet will help you get the details of the key stored on the metadata server. Please note these details can be customised and
...
[examples repository](https://github.com/tkey/tkey-mpc-examples/tree/main/tkey-mpc-web/tkey-mpc-react-popup-example). Check it out and try running it
locally yourself!
